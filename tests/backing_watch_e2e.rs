...
/// debounce window, and asserts cache eviction side effects. Debounce is 150 ms.
mod common;
...
async fn wait_for_cached(
    ...
    loop {
        if harness.cache_mgr.is_cached(rel) == expect {
            return true;
        }
...
#[tokio::test]
async fn backing_modify_evicts_cache_entry() {
    ...
    let cached = wait_for_cached(&h, &rel, true, Duration::from_secs(5)).await;
    ...
    // Allow extra time for spawn_blocking(drop_stale) to finish.
    tokio::time::sleep(Duration::from_millis(200)).await;

    assert!(
        !h.cache_mgr.is_cached(&rel),
        "cache entry should be evicted after backing modify"
    );
}
...
#[tokio::test]
async fn backing_rename_replace_evicts_cache_entry() {
    let h = BackingWatchHarness::new(DEBOUNCE_MS).unwrap();
    ...
    assert!(
        !h.cache_mgr.is_cached(&rel),
        "cache entry should be evicted after backing rename-replace"
    );
}
...
async fn unrelated_files_not_evicted() {
    ...
    std::fs::write(h.backing_path().join(&rel_a), b"episode one content padding_").unwrap();
    std::fs::write(h.backing_path().join(&rel_b), b"episode two content padding_").unwrap();

    h.cache_io.submit_cache(rel_a.clone()).await;
    h.cache_io.submit_cache(rel_b.clone()).await;
    let a_cached = wait_for_cached(&h, &rel_a, true, Duration::from_secs(5)).await;
    let b_cached = wait_for_cached(&h, &rel_b, true, Duration::from_secs(5)).await;
    assert!(a_cached && b_cached, "both files should be cached");

    std::fs::write(h.backing_path().join(&rel_a), b"episode one content changed_").unwrap();

    h.wait_for_debounce().await;
    tokio::time::sleep(Duration::from_millis(200)).await;

    assert!(!h.cache_mgr.is_cached(&rel_a), "ep01 should be evicted");
...
async fn root_auto_seeded_subdir_requires_lookup() {
    ...
    std::fs::write(h.backing_path().join(&root_rel),   b"root content padded________").unwrap();
    std::fs::write(h.backing_path().join(&subdir_rel), b"subdir content padded______").unwrap();

    h.cache_io.submit_cache(root_rel.clone()).await;
    h.cache_io.submit_cache(subdir_rel.clone()).await;
    assert!(wait_for_cached(&h, &root_rel,   true, Duration::from_secs(5)).await);
    assert!(wait_for_cached(&h, &subdir_rel, true, Duration::from_secs(5)).await);

    std::fs::write(h.backing_path().join(&root_rel),   b"root content changed_______").unwrap();
    std::fs::write(h.backing_path().join(&subdir_rel), b"subdir content changed_____").unwrap();

    h.wait_for_debounce().await;
    tokio::time::sleep(Duration::from_millis(200)).await;

    assert!(!h.cache_mgr.is_cached(&root_rel),
        "root watch auto-seeded — eviction must fire");
    assert!(h.cache_mgr.is_cached(&subdir_rel),
        "subdir watch not seeded — mutation must be invisible");
}

#[tokio::test]
async fn debounce_coalesces_burst_into_single_eviction() {
    let h = BackingWatchHarness::new(DEBOUNCE_MS).unwrap();
    let rel = PathBuf::from("coalesce.mkv");

    std::fs::write(h.backing_path().join(&rel), b"initial content padded_____").unwrap();
    h.cache_io.submit_cache(rel.clone()).await;
    assert!(wait_for_cached(&h, &rel, true, Duration::from_secs(5)).await);

    // Five rapid writes — all land inside the leading-edge debounce window.
    for i in 0u8..5 {
    ...
    assert!(!h.cache_mgr.is_cached(&rel), "burst should evict exactly once");
    ...
    h.wait_for_debounce().await;
    tokio::time::sleep(Duration::from_millis(100)).await;
    assert!(h.cache_mgr.is_cached(&rel), "no residual window — re-cached file must survive");
}
...
async fn disabled_watcher_does_not_evict() {
    ...
    assert!(h.cache_mgr.is_cached(&rel), "watcher disabled — cache entry must persist");
}

#[tokio::test]
async fn attrib_change_evicts_cache_entry() {
    let h = BackingWatchHarness::new(DEBOUNCE_MS).unwrap();
    ...
    assert!(!h.cache_mgr.is_cached(&rel), "IN_ATTRIB must evict cache entry");
}
...
async fn lru_evicted_watch_goes_blind() {
    ...
    let _ = std::fs::read_dir(h.mount_path().join("dir_00")).unwrap().count();
    ...
    for i in 1..=65usize {
        ...
        let _ = std::fs::read_dir(h.mount_path().join(&name)).unwrap().count();
    }
    ...
    assert!(h.cache_mgr.is_cached(&rel),
