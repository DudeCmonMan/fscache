...
fn mtime_nanoseconds_preserved() {
    ...
    assert_eq!(
        meta.mtime_nsec(),
        123_456_789,
        "mtime nanoseconds should be preserved (not truncated to 0)"
    );
}
...
fn atime_nanoseconds_preserved() {
    ...
    let meta =
        std::fs::metadata(h.mount_path().join("atimed.mkv")).expect("metadata through FUSE failed");

    assert_eq!(meta.atime(), 1_600_000_000, "atime seconds should match");
    assert_eq!(
        meta.atime_nsec(),
        987_654_321,
        "atime nanoseconds should be preserved"
    );
}
...
fn file_permissions_match_backing() {
    ...
    let mount_meta =
        std::fs::metadata(h.mount_path().join("perms.mkv")).expect("metadata through FUSE failed");
    ...
    assert_eq!(
        mount_meta.uid(),
        backing_meta.uid(),
        "uid should match backing file"
    );
    assert_eq!(
        mount_meta.gid(),
        backing_meta.gid(),
        "gid should match backing file"
    );
}
...
fn symlink_readlink_works() {
    ...
    assert!(
        ...
        "FUSE should report link.mkv as a symlink"
    );

    let target =
        std::fs::read_link(h.mount_path().join("link.mkv")).expect("read_link through FUSE failed");
    assert_eq!(
        target,
        h.backing_path().join("real.mkv"),
        "readlink should return the correct symlink target"
    );
...
fn directory_symlink_traversal() {
    ...
    assert!(
        link_meta.file_type().is_symlink(),
        "FUSE should report linked_dir as a symlink"
    );
    ...
    let entries: Vec<_> = std::fs::read_dir(h.mount_path().join("linked_dir"))
        ...
        .collect();
    assert!(
        entries.contains(&"episode.mkv".to_string()),
        "directory symlink traversal should expose underlying files"
    );
...
fn statfs_returns_valid_data() {
    ...
    assert_eq!(
        rc, 0,
        "statvfs on FUSE mount should succeed (df / free-space checks would fail otherwise)"
    );
    assert!(buf.f_bsize > 0, "block size must be positive");
    assert!(buf.f_blocks > 0, "total blocks must be positive");
    assert!(
        buf.f_bavail <= buf.f_blocks,
        "available blocks cannot exceed total blocks"
    );
    assert!(buf.f_namemax > 0, "max filename length must be positive");
}

/// Cached regular-file getattr is cache-authoritative: once a file is cached,
/// FUSE metadata comes from the stored source snapshot rather than synchronously
/// statting the backing store.
#[tokio::test]
async fn cached_getattr_returns_stored_source_mtime() {
    ...
    let cache_dest = h.cache_path().join(rel);
    {
        ...
        fscache::cache::io::copy_for_tests(&bs, rel, &cache_dest).unwrap();
    }
    let initial_meta = std::fs::metadata(&backing_path).unwrap();
    h.cache_mgr().mark_cached(
        rel,
        fscache::cache::db::SourceMetadata::from_metadata(&initial_meta),
    );
    ...
    let meta = std::fs::metadata(h.mount_path().join(rel)).expect("getattr through FUSE failed");

    assert_eq!(
        meta.mtime(),
        1_700_000_000,
        "cached getattr should return stored source mtime, not the later backing mtime"
    );
}
...
async fn cached_file_preserves_metadata() {
    ...
    let cache_dest = h.cache_path().join("tv/Show/S01E01.mkv");
    {
        ...
        fscache::cache::io::copy_for_tests(
            &bs,
            std::path::Path::new("tv/Show/S01E01.mkv"),
            &cache_dest,
        )
        .expect("copy_for_tests failed");
    }

    assert!(
        cache_dest.exists(),
        "cached file should exist after copy_to_cache"
    );
